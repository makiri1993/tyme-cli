...
exports.weekNumberYearSat = exports.weekNumberYearSun = exports.weekNumberYear = exports.weekNumberSat = exports.weekNumberSun = exports.weekNumber = exports.dayOfYear = void 0;
...
var MINUTE = 60000;
var DAY = 86400000;
var WEEK = 604800000; // = 7 * 24 * 60 * 60 * 1000 = 7 days in ms
...
var tzDiff = function tzDiff(first, second) {
  return (first.getTimezoneOffset() - second.getTimezoneOffset()) * MINUTE;
};
/**
...
var dayOfYear = function dayOfYear() {
...
var weekNumber = function weekNumber() {
  var date = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : new Date();
  // day 0 is monday
  var day = (date.getDay() + 6) % 7; // get thursday of present week

  var thursday = new Date(date);
  thursday.setDate(date.getDate() - day + 3); // set 1st january first

  var firstThursday = new Date(thursday.getFullYear(), 0, 1); // if Jan 1st is not a thursday...

  if (firstThursday.getDay() !== 4) {
    firstThursday.setMonth(0, 1 + (11
    /* 4 + 7 */
    - firstThursday.getDay()) % 7);
  }

  var weekNumber = 1 + Math.floor((thursday - firstThursday + tzDiff(firstThursday, thursday)) / WEEK);
  return weekNumber;
};
/**
...
var weekNumberSun = function weekNumberSun() {
  var date = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : new Date();
  // get sunday of present week
  var sunday = new Date(date);
  sunday.setDate(date.getDate() - date.getDay()); // set 1st january first

  var firstSunday = new Date(sunday.getFullYear(), 0, 1); // if Jan 1st is not a sunday...

  if (firstSunday.getDay() !== 0) {
    firstSunday.setMonth(0, 1 + (7 - firstSunday.getDay()) % 7);
  }

  var weekNumber = 1 + Math.floor((sunday - firstSunday + tzDiff(firstSunday, sunday)) / WEEK);
  return weekNumber;
};
/**
...
exports.weekNumberSun = weekNumberSun;

var weekNumberSat = function weekNumberSat() {
  var date = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : new Date();
  // day 0 is saturday
  var day = (date.getDay() + 1) % 7; // get saturday of present week

  var saturday = new Date(date);
  saturday.setDate(date.getDate() - day); // set 1st january first

  var firstSaturday = new Date(saturday.getFullYear(), 0, 1); // if Jan 1st is not a saturday...

  if (firstSaturday.getDay() !== 6) {
    firstSaturday.setMonth(0, 1 + (13 - firstSaturday.getDay()) % 7);
  }

  var weekNumber = 1 + Math.floor((saturday - firstSaturday + tzDiff(firstSaturday, saturday)) / WEEK);
  return weekNumber;
};
/**
...
exports.weekNumberSat = weekNumberSat;

var getYear = function getYear(date, week) {
...
var weekNumberYear = function weekNumberYear(date) {
  date = new Date(date);
...
exports.weekNumberYear = weekNumberYear;

var weekNumberYearSun = function weekNumberYearSun(date) {
  date = new Date(date);
  var week = weekNumberSun(date);
  var year = getYear(date, week);
  var day = (date.getDay() + 1) % 7 || 7;
  return {
    year: year,
    week: week,
    day: day
...
exports.weekNumberYearSun = weekNumberYearSun;

var weekNumberYearSat = function weekNumberYearSat(date) {
  date = new Date(date);
  var week = weekNumberSat(date);
  var year = getYear(date, week);
  var day = (date.getDay() + 2) % 7 || 7;
  return {
    year: year,
    week: week,
    day: day
...
exports.weekNumberYearSat = weekNumberYearSat;